    ...
    const regionalSeatMap = {
      'AMAZONAS':1,'ANCASH':1,'ÁNCASH':1,'APURIMAC':1,'APURÍMAC':1,'AREQUIPA':1,'AYACUCHO':1,'CAJAMARCA':1,'CALLAO':1,'CUSCO':1,'HUANCAVELICA':1,'HUANUCO':1,'HUÁNUCO':1,'ICA':1,'JUNIN':1,'JUNÍN':1,'LA LIBERTAD':1,'LAMBAYEQUE':1,'LIMA METROPOLITANA':4,'LIMA PROVINCIAS':1,'LORETO':1,'MADRE DE DIOS':1,'MOQUEGUA':1,'PASCO':1,'PIURA':1,'PUNO':1,'SAN MARTIN':1,'SAN MARTÍN':1,'TACNA':1,'TUMBES':1,'UCAYALI':1,'RESIDENTES EN EL EXTRANJERO':1
    };
    ...
    function normalizeCamera(data, key){
      ...
      out.circunscripciones = Array.isArray(out.circunscripciones) ? out.circunscripciones : [];
      ...
      if(key === 'senadoNacional' && !out.circunscripciones.length) out.circunscripciones=[{name:'NACIONAL',seats:30,votes:[]}];
      if(key === 'andino' && !out.circunscripciones.length) out.circunscripciones=[{name:'NACIONAL',seats:5,votes:[]}];
      if(key === 'senadoRegional' && !out.circunscripciones.length){out.circunscripciones=Object.entries(regionalSeatMap).filter(([name])=>!['ÁNCASH','APURÍMAC','HUÁNUCO','JUNÍN','SAN MARTÍN'].includes(name)).map(([name,seats])=>({name,seats,votes:[]}));}
      return out;
    }
    ...
    function precomputedAllocation(data, scenario, circs){
    ...
    function aggregateForCurrentSelection(){
      ...
      let seatsNoBarrier=precomputedAllocation(data,'noBarrier',circs)||allocateAcrossCircunscripciones(circs,null);
      const fallbackEligible=qualifyingParties(data,camera);
      let seatsBarrier=precomputedAllocation(data,'barrier',circs)||allocateAcrossCircunscripciones(circs,fallbackEligible);
    ...
    function renderSeatSquare(detail,partyMap,index,pending=false){const party=typeof detail==='string'?detail:(detail?.party||'Sin asignar');const meta=partyMap.get(party);const color=meta?.color||(party==='Sin asignar'?'#d8e0eb':party==='Pendiente'?'#edf2f7':hashColor(party));const label=party==='Sin asignar'?'—':party==='Pendiente'?'':(meta?.short||initials(party));const logo=meta?.logo||meta?.logoUrl||meta?.urlLogo||'';const content=logo?`<img class="seat-logo" src="${esc(logo)}" alt="${esc(party)}" onerror="this.replaceWith(document.createTextNode('${esc(label)}'))">`:label;const cls=(pending?' pending-seat':(party==='Sin asignar'?' empty-seat':''))+(logo?' has-logo':'');return `<span class="seat-square${cls}" style="background:${color}" tabindex="0" aria-label="${esc(party)}">${content}${tooltipHTML(typeof detail==='string'?{party,candidate:null}:detail,meta,index)}</span>`;}
